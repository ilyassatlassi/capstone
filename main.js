...
const featured = [
  {
    ...
    desc: '“Football plays a vital role in the world, perhaps even more important than politics. People who hate each other sometimes come together thanks to football.”',
  },
  {
...
for (let i = 0; i < featured.length && i < 3; i += 1) {
  ...
  maintextdivp.innerText = featured[i].desc;
  maintextdiv.append(
    maintextdivh1,
    maintextdivh3,
    maintextdivp,
...
for (let i = 3; i < featured.length && i < 6; i += 1) {
  const maindivcard = document.createElement('div');
  maindivcard.classList.add('maincard');
  const imagediv = document.createElement('div');
  imagediv.classList.add('imagediv');
